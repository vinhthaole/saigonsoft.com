...
const customerNavItems = [
    ...
    { href: '/profile/downloads', label: 'Tải về & Giấy phép', icon: Download },
    { href: '/profile/wishlist', label: 'Danh sách Yêu thích', icon: Heart },
    { href: '/profile/loyalty', label: 'Khách hàng Thân thiết', icon: Trophy },
]
...
export default function ProfileLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (!loading && !user) {
            router.push('/login');
        } else if (!loading && userProfile?.role === 'reseller') {
                    ...
                    <aside className="md:col-span-1 no-print">
                        <nav className="flex flex-col space-y-2">
                           {customerNavItems.map(item => (
                             <Button
                                key={item.href}
                                variant={pathname.startsWith(item.href) && (item.href !== '/profile' || pathname === '/profile') ? 'default' : 'ghost'}
                                asChild
